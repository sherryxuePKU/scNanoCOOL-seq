...
def show_help():
   print >>sys.stderr,      "\n\tpython ",sys.argv[0]," /datd/huboqiang/test_NOM/02.SingleC/mESC_gF28_1/singleC/chr10.ACG.TCG.bed.gz /datd/huboqiang/test_NOM/02.SingleC/mESC_gF28_1/singleC/chr10.ACG.TCG.bed.CT_total.xls"

class NDR(object):
    def __init__(self, chrom, count_u, count_m, cutoff = 1e-20, bin_len=200, step_len=20, dist_len=140 ):
        self.chrom    = chrom
...
#        np_idx = np.array([ self.NDR_info['idx'].index(i) for i in l_val ],dtype="int")
        ...
        print "%s\t%d\t%d\t%d\t%s\t%s\t%s" % (self.chrom, self.NDR_info['beg'], self.NDR_info['end'], np_length ,str_p, str_u, str_m)
...
#        CUTOFF = int(sys.argv[4])
    ...
    f_cntfile = open(cntfile, "r")    
    l_line=  f_cntfile.readlines()
    
    CHROM =infile.split("/")[-1].split(".")[0]
    COUNT_U = int(l_line[0].split()[0])
    COUNT_M = int(l_line[0].split()[1])
