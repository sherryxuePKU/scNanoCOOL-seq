...
class BamFileReadParser_NOMe_indel:
    """
    Used to simplify the opening and reading from BAM files. BAMs must be coordinate sorted and indexed.

    :Example:
        >>> from clubcpg.ParseBam_NOMe_indel import BamFileReadParser_NOMe
        >>> parser = BamFileReadParser("/path/to/data.BAM", quality_score=20)
    ...
    def parse_reads(self, chromosome: str, start:int , stop: int, Ctype: str):
        """
        ...
        for read in reads:
            ...
            it=re.finditer(pattern,ref_seq)
            for m in it:
        ...
        for read_cpg in read_cpgs:
            ...
            for pos, tag in read_cpg:
                ...
                if pos and (pos > start) and (pos <= stop): ## changed in 0916
                    temp.append((pos, tag))
                    found_cpg_count += 1
