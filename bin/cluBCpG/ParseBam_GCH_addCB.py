...
class BamFileReadParser_GCH:
    ...
    def parse_reads(self, chromosome: str, start:int , stop: int):
        ...
        for read in reads:
            ...
            it=re.finditer("GC[ATC]",ref_seq)
            for m in it:
                start_pos.append(m.start() + start + 1)
            ## GCH in negative strand
            # it=re.finditer("GC[AT]",ref_seq)
            # for m in it:
            #     start_neg.append(m.start() + start + 1)
            
            true_tag="a"
            reduced_read = []
            for pair, tag in zip(read.get_aligned_pairs(with_seq=True), read.get_tag("XM")):
                if read.flag==0 and pair[1] in start_pos:
                    if pair[2]=="C":true_tag="Z"
                    if pair[2]=="c":true_tag="z"
                    reduced_read.append((pair[1], true_tag, read.get_tag("CB")))
                # if read.flag==16 and pair[1] in start_neg:
        ...
        for read_cpg in read_cpgs:
            read_cpg_index +=1
            temp = []
            for pos, tag, cell_name in read_cpg:
                if pos and (pos > start) and (pos <= stop) and ((tag == 'Z') or (tag == 'z')):
                    temp.append((pos, tag, cell_name))
                    found_cpg_count += 1
    ...
    def create_matrix(self, read_cpgs):
        ...
        for data in read_cpgs:
            ...
            cell_name_set = set()
            cell_name_keys = []
            for pos, status, cell_name in data:
                num_positions += 1
                # cell_pos = str(pos) + "-" + cell_name
            ...
            if dup_flag:
                ...
                if num_positions > 0 and len(cell_name_set)==1:
                    cell_name_keys.append(list(cell_name_set)[0])
                    series.append(pd.Series(statues, positions))

        try:
            matrix = pd.concat(series, axis=1, ignore_index=True, keys=cell_name_keys)
