...
class haploTagging:
    ...
    """
    def __init__(self,
                 bam_file, snp_file, output_directory, min_snp=2, hap_fc=2, number_of_processors=2):
        """
        ...
        :param output_directory: filename of output
        :param min_snp: minimum SNPs covered by single reads
        :param hap_fc: minimum fold change of haps, e.g. g1_count/g2_count
        ...
        self.min_snp = int(min_snp)
        self.hap_fc = int(hap_fc)
    ...
    def assign_read(self, qn):
        ...
        '''
        add 230605, to improve the precision of haplotagging
            with all the reads tagged by at least 1 SNP:
            1) precision = 4065/(4065+37075) ≈ 9.9%;
            2) tagging_ratio = (4065+37075)/113328 ≈ 36%

            with all the reads tagged by at least 2 SNP:
            1) precision needed
            2) tagging_ratio needed
        
        '''
        if g1 >=2:
            # if g2==0: ref_qname.append(qn)
            # elif g1/g2 >= 2: ref_qname.append(qn)
            if g2==0: ref_qname = qn
            elif g1/g2 >= 2: ref_qname = qn
        if g2 >=2:
            if g1==0: alt_qname = qn
            elif g2/g1 >= 2: alt_qname = qn
    ...
    def analyze_snps(self):
        ...
        # read_results = []
        # with Pool(processes=self.number_of_processors) as pool:
        #     results = pool.map_async(self.assign_read, full_qnames)
        #     track_progress(results)

        #     results = results.get()
            # read_results.extend([results])
        # added 230606, use numpy to accelerate
        ...
        flat_arr = np.array(flat_res)
        uniq_qn, count_gt = np.unique(flat_arr, return_counts = True,axis=0)
        np_long = np.concatenate((uniq_qn, np.transpose([count_gt])), axis = 1)

        rows, row_pos = np.unique(np_long[:, 0], return_inverse=True)
        cols, col_pos = np.unique(np_long[:, 1], return_inverse=True)
        arr = np.zeros((len(rows), len(cols)), dtype = int)
        arr[row_pos, col_pos] = np_long[:, 2]

        df = pd.DataFrame(arr)
        df.columns = cols
        df.index = rows
        
        condi = [(df['genome1']>=self.min_snp) & (df['genome2']==0), \
            (df['genome2']>0) & (df['genome1']>=self.hap_fc*df['genome2']),\
            (df['genome2']>=self.min_snp)&(df['genome1']==0),\
            (df['genome1']>0) & (df['genome2']>=self.hap_fc*df['genome1'])]
        
        choices = ['genome1', 'genome1','genome2','genome2' ]
        df['gt']=np.select(condi, choices, default = np.nan)
        
        ref_qname = list(df[df['gt']=='genome1'].index)
        alt_qname = list(df[df['gt']=='genome2'].index)
        
        # read_results = np.array(results)
        # ref_qname = list(read_results[:,0])
        # alt_qname = list(read_results[:,1])
